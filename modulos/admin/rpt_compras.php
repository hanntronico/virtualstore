  ...
  function envia_rpt() {
  	...
  	var opc01 = document.getElementsByName("opt1");
		...
		if (opc01[0].checked) {
			...
			content.fadeIn('slow').load("proc_rpt04.php?pr1=all");
		};
  	...
  	if (opc01[1].checked) {
	  	...
	  	if (parabus=="") {
			...
			var content = jQuery("#view_rpt");
			content.fadeIn('slow').load("proc_rpt04.php?pr1="+param.substring(0,(param.length-3)));
	  };	
  	...
  	if (opc01[2].checked) {
		...
		if(Number(f_ini)>Number(f_fin)){
	  	...
	  	var content = jQuery("#view_rpt");
			content.fadeIn('slow').load("proc_rpt04.php?pr1=em&fci="+fini+"&fcf="+ffin);
  	};
  	...
  	if (opc01[3].checked) {
		...
		if(Number(f_ini)>Number(f_fin)){
	  	...
	  	var content = jQuery("#view_rpt");
			content.fadeIn('slow').load("proc_rpt04.php?pr1=vnc&fci="+fini+"&fcf="+ffin);
  	};
 	...
 	function verprint(url) {
		...
		// alert(parabus);
	  	var opc01 = document.getElementsByName("opt1");
			...
			if (opc01[0].checked) {
				printview('print_rpt04.php?pr1=all');
			};

	  	if (opc01[1].checked) {
				printview('print_rpt04.php?pr1='+parabus);
	  	};	

	  	if (opc01[2].checked) {
	  		printview('print_rpt04.php?pr1=em&fci='+fini+'&fcf='+ffin);
	  	};

	  	if (opc01[3].checked) {
	  		printview('print_rpt04.php?pr1=vnc&fci='+fini+'&fcf='+ffin);
	  	};
	}	
						...
						<td>
				  		<table cellpadding="0" cellspacing="0" border="0" class="stdtable">
				    		<tbody>
				      		<tr>
						        <td colspan="2">
						        	<input type="radio" name="opt1" value="1" checked="checked">Todas las compras
						         	&nbsp;&nbsp;&nbsp;
